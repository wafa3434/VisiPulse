...
# ---------------------------------------------------------------------------
# 1. الإعدادات والتحقق الأمني الحرج من البيئة وحوكمة البيانات
# ---------------------------------------------------------------------------
load_dotenv()
...
# ---------------------------------------------------------------------------
# 2. التسجيل الأمني (Audit Logging) والتشفير وتطهير المدخلات والسياسات
# ---------------------------------------------------------------------------
logging.basicConfig(
    filename=LOG_FILE,
    level=logging.INFO,
...
def validate_password_strength(password: str) -> tuple[bool, str]:
    """فرض سياسة تعقيد كلمات المرور متوافقة مع معايير الأمن السيبراني NCA."""
    if len(password) < 8:
        return False, "يجب ألا تقل كلمة المرور عن 8 خانات."
    ...
    if not re.search(r"[@$!%*?&]", password):
        return False, "يجب أن تحتوي كلمة المرور على رمز خاص واحد على الأقل (مثل @$!%*?&)."
    return True, ""
...
def verify_login(username: str, password: str):
    ...
    if not row or not row["is_active"]:
        log_action(username, "محاولة دخول فاشلة", "حساب غير موجود أو معطل")
    ...
    if row["locked_until"] and datetime.now() < datetime.fromisoformat(row["locked_until"]):
        log_action(username, "محاولة دخول على حساب مقفل", "تجاوز الحد الأقصى للمحاولات")
    ...
    if attempts >= MAX_LOGIN_ATTEMPTS:
        ...
        log_action(username, "محاولة دخول فاشلة", f"كلمة مرور غير صحيحة (المحاولة {attempts})")
    return None, "كلمة المرور أو اسم المستخدم غير صحيح."

# ---------------------------------------------------------------------------
# 5. دوال العمليات الإدارية وتشفير التقارير مع التحقق البرمجي من الصلاحيات
# ---------------------------------------------------------------------------
def check_authorization(required_roles: list[str], user_role: str):
    """التحقق المباشر من الصلاحيات على مستوى الدوال لمنع تصعيد الصلاحيات (Privilege Escalation)."""
    if user_role not in required_roles:
        log_action(user_role, "انتهاك صلاحيات الوصول", f"تم رفض تنفيذ دالة تتطلب: {required_roles}")
...
# ---------------------------------------------------------------------------
# تطبيق واجهات الأمان بناءً على الصلاحيات (Least Privilege)
# ---------------------------------------------------------------------------
try:
