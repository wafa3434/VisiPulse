...
def verify_login(username: str, password: str):
    ...
    if row["locked_until"] and datetime.now() < datetime.fromisoformat(row["locked_until"]):
        return None, "الحساب مقفل مؤقتاً لحماية أمن المنظومة."
...
# ---------------------------------------------------------------------------
# 5. واجهة الاستخدام مع الشعار (من GitHub)، خيار الترجمة، واسم النظام فقط
# ---------------------------------------------------------------------------
...
# رابط الشعار المباشر من GitHub (تأكدي من استبدال اسم المستخدم واسم المستودع بالصحيح)
LOGO_URL = "https://raw.githubusercontent.com/Wafaa-Aglan/VisiPulse/main/logo.Jpej"
...
if "user" not in st.session_state: 
    ...
    if time.time() - st.session_state.last_activity > (SESSION_IDLE_TIMEOUT_MIN * 60):
        st.warning("تم إنهاء الجلسة تلقائياً نظراً لعدم النشاط.")
        st.session_state.user = None
        st.rerun()
    st.session_state.last_activity = time.time()

if st.session_state.user is None:
    # عرض الشعار واسم النظام فقط
