        ...
        if it_passcode == "it123":
            ...
            if "E-health" in sub_tab or "الصحة الإلكترونية" in sub_tab:
                ...
                if "وحدة الجودة" in quality_view or "Quality Management" in quality_view:
                    ...
                    st.info("يتخذ مدير الجودة هنا قرارات مراجعة مستويات الخدمة (SLA)، تقييم رضا المستفيدين، واعتماد معايير الأمان الرقمي بناءً على الرسوم البيانية أدناه.")
                    ...
                    with col_q1:
                        ...
                        st.bar_chart(sla_df)
                    with col_q2:
                        st.markdown("##### مؤشر رضا المستفيدين والموظفين عن النظام")
            ...
            # 4. قسم البنية التحتية
                ...
                st.warning("تنبيه استباقي: تم رصد ضغط عالي على سويتش مبنى العيادات، وتم إرسال تنبيه فوري لفريق البنية التحتية والأمن السيبراني للمراجعة اليدوية (وفقاً للسياسات الأمنية المعتمدة).")
