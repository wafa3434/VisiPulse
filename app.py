...
with tab1:
    ...
    with st.form("proactive_alert_form"):
        dept = st.selectbox("القسم:", ["العناية المركزة (ICU)", "الطوارئ", "المختبر", "العيادات الخارجية"])
...
with tab2:
    mgmt_pass = st.text_input("أدخل كود الإدارة:", type="password")
...
with tab3:
    it_pass = st.text_input("أدخل كود الدعم الفني:", type="password")
    if it_pass == "it123":
        ...
        if st.session_state.tickets:
            ...
            if st.button("استخراج تقرير CSV"):
                csv = df.to_csv(index=False).encode('utf-8-sig')
                st.download_button("تحميل التقرير", data=csv, file_name="Proactive_Tickets.csv", mime="text/csv")
        else:
